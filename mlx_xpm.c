...
void* mlx_int_parse_xpm(t_xvar* xvar, void* info, int info_size, char* (*f)())
{
    ...
    while (i--)
    {
        ...
        if ((rgb_col = mlx_int_get_text_rgb(tab[j], tab[j+1]))==-1)
        {
                if (!(clip_data = malloc(4*width*height)) ||   ok, nice size ..
                                !(clip_img = XCreateImage(xvar->display,
        xvar->visual, 1, XYPixmap, 0, clip_data, width, height, 8, (width+7)/8))
        ) RETURN; memset(clip_data, 0xFF, 4*width*height);
        }
        */
        if (method)
        ...
        else
        {
            ...
            colors[i].col =
                rgb_col; // rgb_col>=0?mlx_get_color_value(xvar,rgb_col):rgb_col;
        }
        free(tab);
        tab = (void*)0;
    }
    ...
    GCClipMask|GCFunction| GCPlaneMask, &xgcv); XSync(xvar->display, False);
...
int mlx_int_file_get_rid_comment(char* ptr, int size)
{
    ...
    {
        com_end =
            mlx_int_str_str(ptr + com_begin + 2, "*/", size - com_begin - 2);
        memset(ptr + com_begin, ' ', com_end + 4);
    }
    while ((com_begin = mlx_int_str_str_cote(ptr, "//", size)) != -1)
    {
        com_end =
            mlx_int_str_str(ptr + com_begin + 2, "\n", size - com_begin - 2);
